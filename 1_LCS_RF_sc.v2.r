...
#Npp<-raster("E:\\SIT\\fra\\avNPP2k2009.tif")
...
lucasDB<-cbind(lucasDB, MAT, RAIN, Corg, CNm, irri, Ndep_WD_tx, NPP, EROS, Nfert, PRMT, ET0, "P_E"=RAIN-ET0, EOBid) 
...
###############################################   LU      ########################################################
LU<-ifelse(lucasDB$LC09 %in% c("F00"), as.character(lucasDB$LC12), as.character(lucasDB$LC09))								##bare soil to LC15 if any crop
...
# lucasDB<-subset(lucasDB, lucasDB$K <1500)   										          ########outlayer K
...
set.seed(419)
RF<-randomForest(OC_sc_g_kg ~ s_c_prc + pH_in_H2O + OC_tf + Ndep_WD_tx + MAT + EROS + K, data = LCS, ntree=1000, mtry=4, importance=TRUE, na.action=na.omit)
RF
...
par(mfrow=c(3,3))
 ...
 partialPlot(RF, LCS, K, main=paste0("%IncMSE=", round(importance(RF)[7],1)), xlab="K")
...
set.seed(432)
RFn<-randomForest(N_sc_g_kg ~ s_c_prc + pH_in_H2O + N_tf + Ndep_WD_tx + MAT + EROS + K, data = LCS, ntree=1000, mtry=4, importance=TRUE, na.action=na.omit)
RFn
...
par(mfrow=c(3,3))
 ...
 partialPlot(RFn, LCS, K, main=paste0("%IncMSE=", round(importance(RFn)[7],1)), xlab="K")
...
##layer n2 
pH_H2O<-raster('E:/SIT/caprese/soil/LUCAS/_LUCAS09_int/pH_H2O_20181023.tif') 
 pH_H2O<-resample(pH_H2O, silt_clay)   
...
##layer n7 
K<-raster('E:/SIT/caprese/soil/LUCAS/_LUCAS09_int/K_20181221.tif') 
K<-resample(K, silt_clay)   
...
inputR[[3]]<-N_1k 
names(inputR)<-c('s_c_prc', 'pH_in_H2O',  'N_tf',  'Ndep_WD_tx',  'MAT', 'EROS', 'K') 
